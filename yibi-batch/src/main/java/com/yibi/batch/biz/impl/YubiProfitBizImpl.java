...
public class YubiProfitBizImpl implements YubiProfitBiz {
    ...
    public void calculProfit(List<Account> accounts, CoinManage coinManage, CoinScale coinScale){
        ...
        for (Account account:accounts){
            ...
            if(user == null|| user.getLogintime()==null||user.getState()!=1){
                ...
                if(profit.compareTo(BigDecimal.ZERO)==1){
                    ...
                    if(user.getReferenceid()!=null && user.getReferenceid()>0){
                        ...
                        if(refUser!=null && !StringUtils.isBlank(refUser.getToken()) &&refUser.getState()==1){
                            ...
                            BigDecimal refProfit = BigDecimalUtils.divide(profit,new BigDecimal(6),coinScale.getYubiscale());
                            if(refProfit.compareTo(BigDecimal.ZERO)>0){
                                if(refLastProfit!=null && refLastProfit.getCreatetime().compareTo(TimeStampUtils.getSomeDayTime(0))>=0){
                                    //推荐人今日收益已经存在,则更新
                                    refLastProfit.setInterest(BigDecimalUtils.add(refLastProfit.getInterest(),refProfit));
                                    refLastProfit.setYubitotalprofit(BigDecimalUtils.add(refTotalProfit,refProfit));
                                    yubiProfitService.updateByPrimaryKey(refLastProfit);

                                }else{
                                    YubiProfit refNewProfit = new YubiProfit(refUser.getId(),account.getCointype(),remain,coinManage.getYubirate(),refProfit,BigDecimalUtils.add(refTotalProfit,refProfit));
                                    yubiProfitService.insert(refNewProfit);
                                }
                                accountService.updateAccountAndInsertFlow(refUser.getId(),GlobalParams.ACCOUNT_TYPE_YUBI,account.getCointype(),refProfit,BigDecimal.ZERO,0,"推荐人收益",0);
