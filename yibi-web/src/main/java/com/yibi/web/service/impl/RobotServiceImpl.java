...
public class RobotServiceImpl implements RobotServiceI {
    ...
    @Override
    public Object addRobotTask(Integer id, Integer operId) {
        Json json = new Json();
        Robot robot = this.robotService.selectByPrimaryKey(id);
        if (robot == null){
            json.setMsg("机器人不存在");
        ...
        List<RobotTask> list = this.robotTaskService.selectAll(map);
        if (list.size()>4){
            json.setMsg("已经添加过了！！请核对");
            return json;
        }
        for(int i= 4;i<6;i++){
            String namePre = robot.getCointype()+"_"+i+"_";
            RobotTask task = new RobotTask();
            task.setCointype(robot.getCointype());
            task.setCountmax(0);
            task.setCountmin(0);
            task.setRobotid(robot.getId());
            task.setExcuteuserid(0);
