...
public class RobotController extends BaseController {
    ...
    public  String updateTaskPage(Integer id,Integer coinType,HttpServletRequest request){
        ...
        if (robotTask.getType() > 3){//撤销
            return "/robot/updateTask3";
        }
    ...
    @RequestMapping("addRobotTask")
    @ResponseBody
    public Object addRobotTask (Integer id,HttpSession session){
        Json json = new Json();
        json.setSuccess(true);
        Integer operId = ((SessionInfo)session.getAttribute("loginUser")).getUserid();
        return robotServiceI.addRobotTask(id,operId);
    }
