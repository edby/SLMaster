...
@Component
@Log4j
public class CancelBuyJob extends BaseJob implements Job {
    @Autowired
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        try {
            ...
            if(task == null){
                log.info("【自动撤销买单】-任务不存在："+taskId);
                return ;
            }
            //撤销操作
            Integer times = task.getCountmax();
            if (times == null){
                log.info("【自动撤销买单】-撤销截止时间没设置:"+task.getId());
                return;
            }
            //查询所有订单状态是未成交的买单
            ...
            if (list == null || list.size()== 0){
                log.info("【自动撤销买单】-撤销截止时间范围内，没有可以撤销的订单");
                return;
            }
            for (OrderSpot orderSpot:list){
                ...
                accountService.updateAccountAndInsertFlow(orderSpot.getUserid(),1,orderSpot.getUnitcointype(),
                        orderSpot.getRemain().multiply(orderSpot.getPrice()),BigDecimal.ZERO,task.getExcuteuserid(),"买入撤销",orderSpot.getId());
            }
            log.info("自动撤销买入订单完成！！");
